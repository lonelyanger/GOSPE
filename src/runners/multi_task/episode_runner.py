...
class EpisodeRunner:
    ...
    def run_logskill(self, test_mode=False, nolog=False, pretrain=False):
        ...
        while not terminated:
            ...
            # Pass the entire batch of experiences up till now to the agents
            # Receive the actions for each agent at this timestep in a batch of size 1
            actions,skill,z = self.mac.select_actions_skill(self.batch, t_ep=self.t, t_env=self.t_env,task=self.task, test_mode=test_mode)
            skill_usages.append(skill.tolist())
            z_usages.append(z.tolist())
            reward, terminated, env_info = self.env.step(actions[0])
            episode_return += reward
