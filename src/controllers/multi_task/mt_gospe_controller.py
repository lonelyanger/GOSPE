...
class GOSPEMAC:
    ...
    def select_actions(self, ep_batch, t_ep, t_env, task, bs=slice(None), test_mode=False):
        ...
        agent_outputs = self.forward2(ep_batch, t_ep, task, test_mode=test_mode)
        chosen_actions = self.action_selector.select_action(agent_outputs[bs], avail_actions[bs], t_env, test_mode=test_mode)
        return chosen_actions
    
    def select_actions_skill(self, ep_batch, t_ep, t_env, task, bs=slice(None), test_mode=False):
        # Only select actions for the selected batch elements in bs
        avail_actions = ep_batch["avail_actions"][:, t_ep]
        agent_outputs = self.forward2(ep_batch, t_ep, task, test_mode=test_mode)
        chosen_actions = self.action_selector.select_action(agent_outputs[bs], avail_actions[bs], t_env, test_mode=test_mode)
        ...
        if self.z==None:
            raise "z is None"
        return chosen_actions,self.skill,self.z
    ...
    def forward_skill(self, ep_batch, t, task, test_mode=False, actions=None):
        ...
        agent_outs, self.hidden_states_enc,loss = self.agent.forward_skill(agent_inputs, self.hidden_states_enc, task, actions=actions)
        agent_outs=agent_outs.view(ep_batch.batch_size, self.task2n_agents[task], -1)
        loss['c']=loss['c'].view(ep_batch.batch_size, self.task2n_agents[task], -1)
        loss['w']=loss['w'].view(ep_batch.batch_size, self.task2n_agents[task], -1)
        return agent_outs,loss

    def forward_both(self, ep_batch, t, task, dist_skill=None,test_mode=False):
        agent_inputs = self._build_inputs(ep_batch, t, task)
        
        def reshape(x):
            return x.view(ep_batch.batch_size, self.task2n_agents[task], -1)
        
        if dist_skill is not None:
            z_outs, q_outs, pri_outs, self.hidden_states_enc,w = self.agent.forward_both_z(agent_inputs, self.hidden_states_enc, task,dist_skill)
        else:
            z_outs, q_outs, pri_outs, self.hidden_states_enc,w = self.agent.forward_both(agent_inputs, self.hidden_states_enc, task)
        
        z_outs=reshape(z_outs)
        q_outs=reshape(q_outs)
        pri_outs=reshape(pri_outs)
        w=reshape(w)
        return z_outs, q_outs, pri_outs,w
    ...
    def forward_seq_action(self, ep_batch, skill_repre, t, task, test_model=False):

        agent_seq_inputs = []
        # skill_action = skill_action.reshape(-1, self.skill_dim)
        skill_repre=skill_repre.reshape(-1,skill_repre.shape[-1])
        ...
        agent_seq_outs, self.hidden_states_dec = self.agent.forward_seq_action(agent_seq_inputs, self.hidden_states_dec, task, skill_repre)
    ...
    def forward2(self, ep_batch, t, task, test_mode=False):
        agent_inputs = self._build_inputs(ep_batch, t, task)
        avail_actions = ep_batch["avail_actions"][:, t]

        bs = agent_inputs.shape[0] // self.task2n_agents[task]
     
        agent_outs, self.hidden_states_enc, self.hidden_states_dec, self.skill,self.z = self.agent.forward2(agent_inputs, self.hidden_states_enc, self.hidden_states_dec, task, None)
