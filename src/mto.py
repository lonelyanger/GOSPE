...
from modules.decomposers import sc2_decomposer
import numpy as np
    ...
    with th.no_grad():
        for task in main_args.test_tasks:
            ...
            for i in range(n_test_runs):
                skill_list,z_list=task2runner[task].run_logskill(test_mode=True)
                task_skill_list.append(skill_list)
                task_z_list.append(z_list)
            
            if main_args.save_replay:
                task2runner[task].save_replay()

            task2runner[task].close_env()
            skill_data[task]=task_skill_list
            z_data[task]=task_z_list
    if main_args.save_replay_skill:
        skill_str=json.dumps(skill_data)
        with open('distskill.json', 'w') as f:
            json.dump(skill_str, f)
        z_str=json.dumps(z_data)
        with open('zskill.json', 'w') as f:
            json.dump(z_str, f)
    logger.log_stat("episode", 0, 0)
    logger.print_recent_stats()

def init_tasks2(task_list, main_args, logger):
    # for bruceyang
    ...
    for task in task_list:
        ...
        scheme = {
            ...
            # "state_graph":{"vshape":(task_args.n_agents,task_args.n_agents)},
            # "obs_graph":{"vshape":(task_args.n_agents,task_args.n_agents),"group":"agents"}
        }
        groups = {
...
def train_sequential(train_tasks, main_args, logger, learner, task2args, task2runner, task2offlinedata, t_start=0,
    ...
    batch_size_graph=main_args.batch_size_graph
    ...
    with th.no_grad():
        # 为什么在训练的时候要开环境  runner有个set函数，放的是mac变量，也就是mac train完后可以在test task 验证
        for task in main_args.test_tasks:
    ...
    while t_env < t_max:
        ...
        for task in train_tasks:
            ...
            if pretrain:
                ...
                if hasattr(learner,"pretrain_graph"):
                    task_smp=sample(train_tasks,2)
                    task1=task_smp[0]
                    task2=task_smp[1]
                    
                    task1_episode_sample = task2offlinedata[task1].sample(batch_size_graph)
                    task1_episode_sample.to(task2args[task1].device)

                    task2_episode_sample = task2offlinedata[task2].sample(batch_size_graph)
                    task2_episode_sample.to(task2args[task2].device)

                    learner.pretrain_graph(task1_episode_sample, task1, task2_episode_sample, task2)
            ...
            if terminated is not None and terminated:
                break
            # t_env is not the time in env, is the train epoches
            t_env += 1
...
def graph_compute(args,decomposer:sc2_decomposer.SC2Decomposer,buffer:OfflineBuffer):
    def GetdataFrombufferByKey(buffer:OfflineBuffer,key):
        return buffer.buffer.data[key]
    state_list=GetdataFrombufferByKey(buffer,'state')
    nobs_list=GetdataFrombufferByKey(buffer,'obs')
    nagents=nobs_list.shape[2]
    buffer_size,max_t_limit,_=state_list.shape
    ally_states, enemy_states, _, _=decomposer.decompose_state(state_list)
    
    ally_xy=th.stack([th.tensor(x[:,:,2:4]) for x in ally_states],dim=2)
    enemy_xy=th.stack([th.tensor(x[:,:,1:3]) for x in enemy_states],dim=2)
    unit_xy=th.cat([ally_xy,enemy_xy],dim=2)
    buffer.buffer.data['state_graph']=th.cdist(unit_xy,unit_xy)

    # _, enemy_feats, ally_feats=decomposer.decompose_obs2(th.tensor(nobs_list))
    # for bs_i in range(buffer_size):
    #     for t in range(max_t_limit):
    #         state=state_list[bs_i,t]
            
    
    #         buffer['state_graph']=th.rand(nagents,nagents)
            
    #         nobs=nobs_list[bs_i,t]
    #         for j in range(nagents):
    #             obs_j=nobs[j]
                

    #             buffer['obs_graph'][j]=th.rand(nagents,nagents)
...
def run_sequential(args, logger):
    ...
    # 初始化每个任务的runner buffer
    task2args, task2runner, task2buffer, task2scheme, task2groups, \
        task2preprocess = init_tasks2(all_tasks, main_args,logger)
    ...
    if getattr(main_args, "pretrain", False):
        ...
        for task in main_args.pretrain_tasks:
            ...
            task2offlinedata[task] =graph_compute(main_args,mac.task2decomposer[task],OfflineBuffer(task, main_args.pretrain_tasks_data_quality[task],
        ...
        if hasattr(learner, 'test_pretrain') and hasattr(main_args, 'test_tasks_data_quality'):
            ...
            for task in main_args.test_tasks_data_quality.keys():
                test_task2offlinedata[task] = graph_compute(main_args,mac.task2decomposer[task],OfflineBuffer(task, main_args.test_tasks_data_quality[task],
                                                            data_folder=main_args.offline_data_name,
                                                            offline_data_size=args.offline_data_size,
                                                            random_sample=args.offline_data_shuffle))
    ...
    for task in main_args.train_tasks:
        # create offline data buffer
        task2offlinedata[task] = graph_compute(main_args,mac.task2decomposer[task],OfflineBuffer(task, main_args.train_tasks_data_quality[task],
                                               data_folder=main_args.offline_data_name,
                                               offline_data_size=args.offline_data_size,
                                               random_sample=args.offline_data_shuffle))
